 ...
 *
 * @package Ingenerator
 * @see     spec\Ingenerator\CellarSpec
 */
class Cellar
{
     ...
     * @param array  $options see Cellar::$default_options for available options
     */
    ...
    public function generate($lifetime = NULL, $extra_values = [])
    {
        ...
        $expires = $this->calculateExpiration($lifetime);
     ...
     * @return int
     */
    protected function calculateExpiration($lifetime)
    {
        return \time() + $lifetime;
    }
    ...
    {
        return ! ($this->hasTampered($token_string, $extra_values) OR $this->hasExpired(
                $token_string
            ));
    }
     ...
     * @return bool
     */
    public function hasTampered($token_string, array $extra_values = [])
    {
     ...
     * @return bool
     */
    public function hasExpired($token_string)
    {
        $parts = $this->parseToken($token_string);
